...
| 질문                              | 답변                                                                                                                                                                |
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PRD 를 얼마나 꼼꼼히 써야 하나요? | 완벽하게 다듬지 마세요. **PRD → 구현 → 피드백** 사이클을 빠르게 도는 것이 핵심입니다. 큰 방향만 맞으면 "확인"으로 넘기고, 세부 사항은 개발 중에 조정합니다.         |
| 기술 스택을 PRD 에 적어야 하나요? | 아니요. 이 워크숍의 기술 스택은 **Nuxt.js (Vue 3)** 이며, 프로젝트 구조에서 자동 인식됩니다.                                                                        |
| 어떤 섹션이 가장 중요한가요?      | **Section 3 (데모 시나리오)** 입니다. AI 에이전트가 이 시나리오를 기반으로 기능 목록을 예상하므로, 사용자가 어떤 화면에서 무엇을 하는지 구체적으로 쓸수록 좋습니다. |
| 기능이 너무 많으면 어떻게 하나요? | **Section 6** 에서 전체 구현 범위를 확인합니다. UI PoC 권장 기능 수는 **5~7개**입니다. 초과하면 `"F8 은 이번 PoC 에서 구현 안할거야"` 등으로 삭제하세요.            |
| Section 7~9 도 직접 써야 하나요?  | 아니요. Section 6 까지 확정하면 나머지는 자동 생성할 수 있습니다 (아래 Step 3 참고).                                                                                |
...
💬 Claude Code 대화창에 평소 쓰던 말투로 PRD 작성을 요청합니다. `@docs/`까지 입력한 뒤 **Tab 키**로 PDF 파일을 자동완성하세요.
...
PRD 가 확정되면 다음 랩으로 넘어갑니다. Lab 3 에서는 PRD 의 각 Feature 를 **ADR (설계 메모)** 로 변환한 뒤 코드로 구현합니다.
