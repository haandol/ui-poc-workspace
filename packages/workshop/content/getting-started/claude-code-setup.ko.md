...
세션을 이어서 진행할 때 Workshop Studio 자격증명이 이미 만료된 경우에는, 위 절차 대신 💬 Claude Code 대화창에서 `/setup-bedrock` 을 실행해 같은 화면을 다시 띄우고 진행자에게 새로 받은 API 키를 등록하면 됩니다. 설치 자체는 유지되므로 키 교체만으로 충분합니다.

::alert[키 재등록이 끝나면 💬 대화창에서 `/model` 로 `Opus 4.6 (us.anthropic.claude-opus-4-6-v1)` 이 선택되어 있는지 확인하세요.]{type="info"}
