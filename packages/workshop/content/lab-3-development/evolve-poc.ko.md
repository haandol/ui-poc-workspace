...
이 워크숍의 핵심은 **"기획 → 구현 → 확인 → 다시 기획" 루프를 최대한 빠르게 돌리는 것"** 입니다. 머릿속에서만 굴리던 아이디어가 **동작하는 화면** 으로 나오는 순간, 거의 항상 _"아, 막상 만들어보니 이건 다른 게 낫겠다"_ 가 따라옵니다 — 본인 안에서도, 내부 리뷰 자리에서도, 개발자 핸드오프 직전에도.

이 랩에서 만든 PoC 는 **그렇게 끊임없이 바뀌는 기획을 무한히 반복 흡수할 수 있도록** 설계되어 있습니다. 핵심은 **ADR 을 결정의 source of truth (기준점)** 로 두는 것입니다 — 기획이 바뀔 때마다 ADR 부터 갱신하고 코드로 내려가면, 변경이 100 번 들어와도 PoC 는 항상 깨끗한 상태에서 다음 변경을 받습니다.
...
```mermaid
flowchart TD
    Review([구현 결과 확인<br/>본인 / 내부 리뷰 / 핸드오프]) --> FB[기획 변경 욕구]
    FB --> Kind{어떤 변경?}
