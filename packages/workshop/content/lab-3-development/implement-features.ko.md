...
Airtable 연동을 설정한 경우, Lab 3 에서는 Claude Code 가 아래 시점에 자동으로 진행 상태를 기록합니다.

- 개발 서버 기동이 확인된 뒤 첫 Feature 구현을 요청했을 때 → `SCAFFOLD-DONE`
- 각 Feature 구현을 끝내고 결과를 보고한 직후 → `F1-DONE`, `F2-DONE`, …
- 데모 준비가 끝났다고 말할 때 → `DEMO-READY`

중복 기록은 자동으로 방지되며, 공유를 원하지 않으면 "진행 상태 공유하지 마"라고 말해 세션 동안 끌 수 있습니다.
