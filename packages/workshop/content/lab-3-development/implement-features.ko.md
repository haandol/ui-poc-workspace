...
/feature-dev @docs/prd/XYZ.alps.md F1 구현해줘.
:::

`/feature-dev`는 Feature 구현 전용 명령어입니다. 이 명령어를 사용하면 Claude가 자동으로:

1. PRD를 분석하고 Feature 의존성을 확인합니다
2. **ADR(Architecture Decision Record)을 작성**하여 구현 방향을 정합니다
3. ADR 내용을 보여주고 확인을 요청합니다
4. 확인 후 코드를 작성합니다

::alert[ADR은 "어떻게 만들지"를 미리 정리하는 설계 문서입니다. Claude가 자동으로 만들어주니 여러분은 내용을 확인하고 승인만 하면 됩니다.]{type="info"}
...
::alert[같은 대화에서 계속 작업 중이면 `"/feature-dev F2 구현해줘."` 처럼 파일 경로 없이 입력해도 됩니다.]{type="info"}
