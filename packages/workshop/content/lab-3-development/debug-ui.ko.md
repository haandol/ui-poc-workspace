...
| 상황                | 프롬프트 예시                                                                     |
| ------------------- | --------------------------------------------------------------------------------- |
| 화면 확인           | "이미 localhost:3000 에 dev 서버가 떠있어. 브라우저로 접속해서 스크린샷 캡처해줘" |
| 화면이 깨져 보일 때 | "현재 페이지를 확인하고 깨진 부분을 수정해줘"                                     |
| 에러가 발생할 때    | "브라우저에 에러가 있는지 확인하고 수정해줘"                                      |
| 디자인 점검         | "페이지 스크린샷을 찍고, PRD와 일치하는지 비교해줘"                               |

::alert["화면 봐줘", "스크린샷 찍어줘", "지금 화면이 이상하게 보이는데 뭐가 문제야?" 같은 간단한 요청만으로도 AI가 문제를 파악할 수 있습니다.]{type="info"}

::alert[브라우저를 여는 기능이 없다고 하면 `/reload-plugins` 를 실행한 뒤 `/mcp` 에서 `chrome-devtools` 플러그인이 connected 상태인지 확인하세요. 자세한 내용은 [MCP 서버 확인](/getting-started/verify-mcp) 페이지를 참고합니다.]{type="warning"}
