...
3. **메인 진입점은 `/` 루트(`app/pages/index.vue`)다.** PRD/ADR 의 핵심 화면(첫 Feature)은 새 페이지를 만들지 말고 `app/pages/index.vue` 를 직접 수정해 구현한다. 자세한 규칙은 아래 [Entry Point Rules](#entry-point-rules) 참조.
4. UI 컴포넌트 수정 전 루트 [DESIGN.md](../../DESIGN.md)와 [docs/design/](../../docs/design/)의 분할 문서를 반드시 읽는다.
5. 커밋 및 코드 스타일 규칙은 루트 [CONTRIBUTING.md](../../CONTRIBUTING.md)를 참조한다.
...
│   └── pages/
│       └── index.vue        # ⭐ 메인 진입점 (`/` 경로) — 핵심 화면은 여기에 구현
...
**메인 진입점은 `/` 루트, 즉 `app/pages/index.vue` 다.** dev 서버를 띄우면 사용자가 가장 먼저 보는 화면이고, 워크숍 데모도 `http://localhost:3000`(루트)를 그대로 공유한다.

- **핵심 화면(첫 Feature / 메인 플로우)은 `app/pages/index.vue` 를 직접 수정해 구현한다.** `pages/dashboard.vue`, `pages/demo.vue` 같은 새 경로를 만들어 거기에 메인 작업을 하지 않는다.
- `app/pages/index.vue` 의 초기 내용은 자리표시용 랜딩 템플릿이다. 첫 Feature 를 구현할 때 이 템플릿을 **덮어쓰는 것이 기본 동작**이다 (보존 대상이 아니다).
- 보조 화면(상세 페이지, 설정 등 메인 플로우에서 분기되는 화면)이 PRD/ADR 상 명확히 필요할 때만 `pages/` 아래 새 경로를 추가하고, 루트에서 그 화면으로 이동(링크/네비게이션)할 수 있게 연결한다.
- 사용자가 별다른 경로를 지정하지 않고 "화면 만들어줘 / 메인 화면 / 이 기능 만들어줘" 라고 하면 **`/` 루트(`index.vue`)를 대상으로 작업한다.**
