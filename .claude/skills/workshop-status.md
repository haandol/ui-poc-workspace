...
2. Bash tool로 환경변수를 셸 프로필에 자동 추가한다 (`.mcp.json`은 수정하지 않는다 — 레포에 키가 노출되므로):

   **Mac/Linux** — 아래 명령을 Bash tool로 실행:
   ```bash
   # 기존에 AIRTABLE_API_KEY가 있으면 제거 후 새로 추가 (멱등성)
   SHELL_PROFILE="${HOME}/.zshrc"
   [ -f "${SHELL_PROFILE}" ] || SHELL_PROFILE="${HOME}/.bashrc"
   grep -v 'AIRTABLE_API_KEY' "${SHELL_PROFILE}" > "${SHELL_PROFILE}.tmp" && mv "${SHELL_PROFILE}.tmp" "${SHELL_PROFILE}"
   echo 'export AIRTABLE_API_KEY="pat_사용자가입력한값"' >> "${SHELL_PROFILE}"
   ```

   **Windows** — 아래 명령을 Bash tool로 실행:
   ```powershell
   [System.Environment]::SetEnvironmentVariable('AIRTABLE_API_KEY', 'pat_사용자가입력한값', 'User')
   ```

3. 참여자 이름을 프로젝트 루트에 `.workshop-participant` 파일로 저장한다 (한 줄, 이름만).

4. 안내: "환경변수가 자동 설정되었습니다. Claude Code를 재시작(`/exit` 후 `claude`)하면 Airtable 연동이 활성화됩니다."

5. 재시작 후 Airtable MCP tool (`list_bases`)로 연결을 확인한다.
...
| -------- | --------- | ---------------- | ------ |
| 홍길동   | 피쳐 개발 | F2 Complete      | 14:35  |
| 김철수   | PRD       | PRD Features     | 14:20  |
| 이영희   | 환경설정  | Env Ready        | 13:50  |
```
...
- `.workshop-participant`와 `.mcp.json`의 API 키 값은 `.gitignore`에 포함되어 커밋되지 않는다
