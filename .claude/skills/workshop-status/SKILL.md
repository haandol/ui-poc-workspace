...
2. `.mcp.json`의 airtable 서버 설정에 API Key를 **환경변수 참조 방식**으로 설정한다.

   먼저 `.mcp.json`을 읽어서 현재 상태를 확인한다:
   - `env.AIRTABLE_API_KEY`가 이미 `${AIRTABLE_API_KEY}`로 되어 있으면 → `.mcp.json` 수정 불필요
   - `env` 필드가 없거나, `AIRTABLE_API_KEY`가 없거나, 하드코딩된 값이면 → 아래처럼 수정:
...
3. 프로젝트 로컬 설정 `.claude/settings.local.json`에 API Key 환경변수를 등록한다:

   `.claude/settings.local.json` 파일을 읽어서 `env` 필드에 `AIRTABLE_API_KEY`를 추가한다.
   - 파일이 없으면 새로 생성한다
   - 이미 `env` 필드가 있으면 `AIRTABLE_API_KEY`만 추가/교체한다
   - 다른 기존 설정은 건드리지 않는다
...
5. 안내: "API Key가 설정되었습니다. `/mcp` 명령어를 실행하여 airtable 서버를 리로드하면 Airtable 연동이 활성화됩니다."

6. MCP 리로드 후 Airtable MCP tool (`list_bases`)로 연결을 확인한다.

7. 연결이 확인되면 SETUP-DONE 마일스톤을 자동으로 기록한다 (Mode 2의 upsert 방식과 동일).
...
2. 아래 순서로 가장 최근 도달한 마일스톤을 역순 판단:
   ...
   - `docs/prd/` 에 `.alps.md` 파일 존재? → PRD-DONE
   - `docs/` 에 PDF 파일 존재? → RESEARCH-DONE
   - `node_modules/` 존재? → SETUP-DONE
3. 판단된 마일스톤을 사용자에게 보여주고 확인 후 Airtable에 전송

**Airtable 기록 (upsert 방식):**

1. `search_records`로 Participant 이름이 일치하는 기존 레코드를 검색한다
2. 기존 레코드가 있으면 → `update_records`로 해당 레코드를 업데이트
3. 기존 레코드가 없으면 → `create_record`로 새 레코드 생성
