...
2. `.mcp.json`의 MCP 서버 설정에 API Key를 **환경변수 참조 방식**으로 설정한다.

   먼저 `.mcp.json`을 읽어서 현재 상태를 확인한다:
   - `airtable.env.AIRTABLE_API_KEY`가 이미 `${AIRTABLE_API_KEY}`로 되어 있으면 → 수정 불필요
   - `asset-generator.env.FAL_KEY`가 이미 `${FAL_KEY}`로 되어 있으면 → 수정 불필요
   - 하드코딩된 값이거나 필드가 없으면 → 아래처럼 수정:
...
6. 안내: "API Key가 설정되었습니다. `/mcp` 명령어를 실행하여 airtable, asset-generator 서버를 리로드하면 연동이 활성화됩니다."
