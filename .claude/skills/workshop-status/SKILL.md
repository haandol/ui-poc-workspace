...
6. 안내: "API Key가 설정되었습니다. `/mcp` 명령어를 실행하여 airtable 서버를 리로드하면 Airtable 연동이 활성화됩니다."

7. MCP 리로드 후 Airtable MCP tool (`list_bases`)로 연결을 확인한다.

8. 연결이 확인되면 SETUP-DONE 마일스톤을 자동으로 기록한다 (`.work-status` append + Airtable upsert).
...
**인자가 없는 경우** — 사용자가 최근 보고한 작업에서 추론:

1. `.workshop-participant` 파일 확인 → 없으면 setup 안내
2. 사용자 최근 발화/작업에서 가장 가까운 마일스톤으로 정규화:
   - "F2 끝났어" / "F2 구현 완료" → `F2-DONE`
   - "dev server 띄웠어" / 랩 3 진입 → `SCAFFOLD-DONE`
   - "데모 준비 끝" → `DEMO-READY`
3. `.work-status` 에 이미 있는 마일스톤이면 사용자에게 알리고 중복 전송은 하지 않는다
4. 판단된 마일스톤을 사용자에게 보여주고 확인 후 기록
...
3. Airtable upsert:
   - `search_records` 로 Participant 일치 레코드 검색
